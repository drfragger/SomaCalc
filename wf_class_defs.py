...
class Enemy:
    def __init__(self,
        ...
        self.props = {
            'hp': df.health_scaling(self),
            'armor': df.armor_scaling(self)
        }
    ...
    def get_resistances(self):
        ...
        for elem in types['Generic']._fields:
...
class Weapon:
    ...
    def get_dps(self, isCrit=False):
        ...
        real_fire_rate = self.stats['Firerate'] / (
            1 + self.stats['Charge Time'])

        if self.stats['Firerate'] == 0:
            real_fire_rate = 1 / self.stats['Charge Time']

        burst = self.totaldmg(
            isCrit=isCrit) * self.stats['Multishot'] * real_fire_rate

        time_elapsed = (
            self.stats['Magazine'] / real_fire_rate) + self.stats['Reload']
        sus_dmg_dealt = self.totaldmg(
            isCrit=isCrit) * self.stats['Multishot'] * self.stats['Magazine']
    ...
    def get_sustained(self, victim, isCrit=False):
        ...
        damage_dealt = self.attack(
            victim, isCrit) * self.stats['Multishot'] * self.stats['Magazine']
    ...
    def get_burst(self, victim, isCrit=False):
        ...
        burst = self.attack(victim,
                            isCrit) * self.stats['Multishot'] * real_fire_rate
        return burst

    def update_elemental_combos(self):

        blst_check = bool(self.dmg['blst']
                          or (self.dmg['ht'] and self.dmg['cld']))
        crsv_check = bool(self.dmg['crsv']
                          or (self.dmg['txn'] and self.dmg['elec']))
        gas_check = bool(self.dmg['gas']
                         or (self.dmg['txn'] and self.dmg['ht']))
        mag_check = bool(self.dmg['mag']
                         or (self.dmg['cld'] and self.dmg['elec']))
        rad_check = bool(self.dmg['rad']
                         or (self.dmg['ht'] and self.dmg['elec']))
        vir_check = bool(self.dmg['vir']
                         or (self.dmg['cld'] and self.dmg['txn']))
