    ...
    def effect(self, weapon_arg):
        if self.modtype == 'damage':
            ...
            if self.stat != 'All':
                ...
                nonzerostat = (weapon_arg.basedmg[self.stat] * (1 + self.bonus)
                               ) - weapon_arg.basedmg[self.stat]

                weapon_arg.dmg[self.stat] += (nonzerostat,
                                              zerostat)[isStatZero]
        ...
        if self.modtype == 'other':
            weapon_arg.stats[
                self.stat] += weapon_arg.basestats[self.stat] * self.bonus
